...
public class GLPolicyService {
    ...
    private GLPolicyMailDetailDto getGlPolicyDetailForPDF(PolicyId policyId, boolean isEndorsementDocument,GroupLifeEndorsement groupLifeEndorsement) {
        ...
        glQuotationDetailDto.setWaiverOfExcessLoadings("0.00");
        glQuotationDetailDto.setWaiverOfExcessLoadingsPercentage("0.00");
        ...
        if (isNotEmpty(groupLifePolicy.getInsureds())) {
            for (Insured insured : groupLifePolicy.getInsureds()) {
                ...
                GLPolicyMailDetailDto.Annexure insuredAnnexure = glQuotationDetailDto.new Annexure(insured.getFirstName() + " " + insured.getLastName()!=null?insured.getLastName():""
    ...
    public GLPolicyMailDetailDto getEndorsementDetails(GroupLifeEndorsement groupLifeEndorsement,GLPolicyMailDetailDto glQuotationDetailDto,Industry industry){
        ...
        if (groupLifeEndorsement.getEndorsement()!=null){
            if (GLEndorsementType.ASSURED_MEMBER_ADDITION.equals(groupLifeEndorsement.getEndorsementType())){
                insureds  = groupLifeEndorsement.getEndorsement().getMemberEndorsement().getInsureds();
            }
            else if (GLEndorsementType.ASSURED_MEMBER_DELETION.equals(groupLifeEndorsement.getEndorsementType())){
                insureds  = groupLifeEndorsement.getEndorsement().getMemberDeletionEndorsements().getInsureds();
            }
            else if (GLEndorsementType.NEW_CATEGORY_RELATION.equals(groupLifeEndorsement.getEndorsementType())){
                insureds  = groupLifeEndorsement.getEndorsement().getNewCategoryRelationEndorsement().getInsureds();
            }
            else if (GLEndorsementType.MEMBER_PROMOTION.equals(groupLifeEndorsement.getEndorsementType())){
                insureds  = groupLifeEndorsement.getEndorsement().getPremiumEndorsement().getInsureds();
            }
        }
        ...
        for (Insured insured :insureds) {
            ...
            if (insuredPremiumDetail!=null) {
                ...
                insuredPlanSA = insuredPlanSA.setScale(2, BigDecimal.ROUND_CEILING);
                GLPolicyMailDetailDto.CoverDetail insuredPlanCoverDetail = glQuotationDetailDto.new CoverDetail(isEmpty(insured.getCategory()) ? "" : insured.getCategory(), "Self", isNotEmpty(planCoverageDetailDtoList) ? planCoverageDetailDtoList.get(0).getPlanName() : "", insuredPlanSA,insured.getNoOfAssured()!=null?insured.getNoOfAssured():insureds.size());
                coverDetails.add(insuredPlanCoverDetail);
                if (isNotEmpty(insured.getCoveragePremiumDetails())) {
                    for (CoveragePremiumDetail coveragePremiumDetail : insured.getCoveragePremiumDetails()) {
                        Map<String, Object> coverageMap = glQuotationFinder.getCoverageDetail(coveragePremiumDetail.getCoverageId().getCoverageId());
                ...
                GLPolicyMailDetailDto.Annexure insuredAnnexure = glQuotationDetailDto.new Annexure(insured.getFirstName() + " " + insured.getLastName()
                        , insured.getNrcNumber()!=null?insured.getNrcNumber():"NA", insured.getGender() != null ? insured.getGender().name() : "", AppUtils.toString(insured.getDateOfBirth()),
                        isEmpty(insured.getCategory()) ? "" : insured.getCategory(), "Self", insured.getDateOfBirth() == null ? "" : AppUtils.getAgeOnNextBirthDate(insured.getDateOfBirth()).toString(), insuredAnnualIncomeInString, insuredBasicPremium.toPlainString(),insuredPremiumDetail.getSumAssured().toPlainString());
                annexures.add(insuredAnnexure);
            }
            if (isNotEmpty(insured.getInsuredDependents())) {
                for (InsuredDependent insuredDependent : insured.getInsuredDependents()) {
                    ...
                    BigDecimal insuredDependentPlanSA = insuredDependentPremiumDetail.getSumAssured();
                    insuredDependentPlanSA = insuredDependentPlanSA.setScale(2, BigDecimal.ROUND_CEILING);
                    GLPolicyMailDetailDto.CoverDetail insuredDependentPlanCoverDetail = glQuotationDetailDto.new CoverDetail(isEmpty(insuredDependent.getCategory()) ? "" : insuredDependent.getCategory(), insuredDependent.getRelationship().description, dependentPlanCoverageDetailDtoList.get(0).getPlanName(), insuredDependentPlanSA,insuredDependent.getNoOfAssured()!=null?insuredDependent.getNoOfAssured():insured.getInsuredDependents().size());
                    coverDetails.add(insuredDependentPlanCoverDetail);
                    if (isNotEmpty(insuredDependent.getCoveragePremiumDetails())) {
                        for (CoveragePremiumDetail dependentCoveragePremiumDetail : insuredDependent.getCoveragePremiumDetails()) {
                            BigDecimal insuredDependentCoverageSA = dependentCoveragePremiumDetail.getSumAssured();
                    ...
                    GLPolicyMailDetailDto.Annexure annexure = glQuotationDetailDto.new Annexure(insuredDependent.getFirstName() + " " + insuredDependent.getLastName()
                            , insuredDependent.getNrcNumber()!=null? insuredDependent.getNrcNumber():"NA", insuredDependent.getGender() != null ? insuredDependent.getGender().name() : "",
                            insuredDependent.getDateOfBirth() == null ? "" : AppUtils.toString(insuredDependent.getDateOfBirth()),
                            isEmpty(insuredDependent.getCategory()) ? "" : insuredDependent.getCategory(), insuredDependent.getRelationship().description, AppUtils.getAgeOnNextBirthDate(insuredDependent.getDateOfBirth()).toString(), "", insuredDependentBasicPremium.toPlainString(),insuredDependentPremiumDetail.getSumAssured().toPlainString());
                    annexures.add(annexure);
                }
            }
        }
    ...
    private List<GLPolicyMailDetailDto.CoverDetail> unifyCoverDetails(List<GLPolicyMailDetailDto.CoverDetail> coverDetails, GLPolicyMailDetailDto glQuotationDetailDto) {
        ...
        List<GLPolicyMailDetailDto.CoverDetail> unifiedCoverDetails = coverDetailsMap.entrySet().stream().map(new Function<Map.Entry<GLPolicyMailDetailDto.CoverDetail, List<GLPolicyMailDetailDto.CoverDetail>>, GLPolicyMailDetailDto.CoverDetail>() {
            ...
            public GLPolicyMailDetailDto.CoverDetail apply(Map.Entry<GLPolicyMailDetailDto.CoverDetail, List<GLPolicyMailDetailDto.CoverDetail>> coverDetailListEntry) {
                ...
                coverDetail = coverDetail.addNoOfEmployeesCovered(coverDetailKey.getNoOfEmployeesCovered());
                return coverDetail;
            }
        }).collect(Collectors.toList());
    ...
    public GLPolicyMailDto getPreScriptedEmail(PolicyId policyId) {
        ...
        GLPolicyMailDto dto = new GLPolicyMailDto(subject, emailBody,mailAddress);
