...
class EnrollmentProcessInformation {
    ...
    public int getTheProductLineProcessTypeValue(ProductLineProcessType productLineProcessType) throws ProcessInfoException {
        Optional<ProductLineProcessItem> optionalProductLineProcessItem = enrollmentProcessItems.stream().filter(new FilterProductLineProcessItem(productLineProcessType)).findAny();
